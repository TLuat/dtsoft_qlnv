...
<?php 
    $id_user = $_SESSION['id'];
    $id = $_GET['id'];
    $sql = "SELECT * FROM theodoikehoach";
    $sql_ct = "SELECT * FROM chitieu";
    $query_ct = mysqli_query($connect,$sql_ct);
    if(isset($_POST['sbm'])){
        if(!empty($_POST['id_chitieu']) && !empty($_POST['chitieudatdc']) && !empty($id)) {
            $id_chitieu = $_POST['id_chitieu'];
            $chitieudatdc = $_POST['chitieudatdc'];
            $id_kehoachgiaoviec = $id;
            $sql = "INSERT INTO theodoikehoach (id_nguoidung, id_chitieu, chitieudatdc, id_kehoachgiaoviec)
            VALUES ('$id_user' ,'$id_chitieu', '$chitieudatdc', '$id_kehoachgiaoviec')";
