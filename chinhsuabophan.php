...
<div class="p-4 sm:ml-64">
    <div class="p-4 mt-14">
        <div class="mb-4 font-bold text-lg uppercase text-center">CHỈNH SỬA BỘ PHẬN</div>
        <div class="mb-4">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-5 rounded">
                <form action="process_chinhsuabophan.php" method="POST">
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <input type="hidden" name="id_bophan" value="<?php echo $bophan['id_bophan'] ?>">
                        <div>
                            <label for="first_name" class="block mb-2 text-sm font-medium">Mã bộ phận</label>
                            <input type="text" name="id_bophan_new" value="<?php echo $bophan['id_bophan'] ?>" id="first_name" class=" border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="id_khuvuc" id="countries" class="border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                ...
                                <?php foreach ($result as $each) : ?>
                                    <option value="<?php echo $each['id_khuvuc'] ?>" <?php echo $each['id_khuvuc'] == $bophan['id_khuvuc'] ? 'selected' : '' ?>><?php echo $each['tenkhuvuc'] ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        <div>
                            <label for="last_name" class="block mb-2 text-sm font-medium">Tên công việc chuyên môn</label>
                            <input type="text" value="<?php echo $bophan['cvchuyenmon'] ?>" name="cvchuyenmon" id="last_name" class=" border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                        </div>
